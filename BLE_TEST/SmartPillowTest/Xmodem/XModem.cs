...
namespace XModem
{
    ...
    public class XModem
    {
        ...
        BluetoothSocket port = null;
        CircularBuffer<byte> ReceivedBuffer;
        ...
        public XModem(BluetoothSocket port, CircularBuffer<byte> bReceiveBuffer)
        {
            this.port = port;
        ...
        private byte readByte()
        {
            ...
            *//* Wait data coming *//*
            while ((this.ReceivedBuffer.Size == 0))
            {
                Thread.Sleep(1);
            };
            *//* Get newest byte *//*
            bByteReceive = this.ReceivedBuffer.PopFront();
            bCharReceive = (char)bByteReceive;
            return bByteReceive;*/

            byte bByteReceive = 0;
            byte[] buffer = new byte[1];
            this.port.InputStream.Read(buffer, 0, 1);
            bByteReceive = buffer[0];
            return bByteReceive;
        }
        //private async Task writeBuffer(byte[] xbuff, int size)
        private void writeBuffer(byte[] xbuff, int size)
        {
            Thread.Sleep(1);
            //await this.port.OutputStream.WriteAsync(xbuff, 0, size);
            this.port.OutputStream.Write(xbuff, 0, size);
        }
        ...
        private void writeByte(byte b)
        {
            byte[] buffer = new byte[1];
            buffer[0] = b;
            //await this.port.OutputStream.WriteAsync(buffer, 0, 1);
            this.port.OutputStream.Write(buffer, 0, 1);
        }
